...
const differencify = new _index2.default({ debug: true });
...
describe('Differencify', () => {
  ...
  it('simple', async () => {
    await differencify.init().newPage().setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).screenshot().toMatchSnapshot().close().end();
  }, 30000);
  ...
  it('Launch new browser per test', async () => {
    await differencify.init().launch({ args: ['--no-sandbox', '--disable-setuid-sandbox'] }).newPage().setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).screenshot().toMatchSnapshot().close().end();
  }, 30000);
  ...
  it('Using result function', async () => {
    await differencify.init().newPage().setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).title().result(title => {
      expect(title).toEqual('Example Domain');
    }).screenshot().toMatchSnapshot().close().end();
  }, 30000);
  it('Using toMatchSnapshot callback for result details', async () => {
    await differencify.init().newPage().setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).title().screenshot().toMatchSnapshot(resultDetail => {
  ...
  it('Context switching when chained', async () => {
    await differencify.init().newPage().tracing.start({ path: 'trace.json' }).page.setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).keyboard.press('Space').tracing.stop().page.screenshot().toMatchSnapshot().close().end();
  }, 30000);
  it('Calling Puppeteer specific functions when chained: console', async () => {
    await differencify.init().newPage().on('console', msg => {
      for (let i = 0; i < msg.args.length; i += 1) {
        expect(`${msg.args[i]}`).toEqual('JSHandle:hello');
      }
    }).evaluate(() => console.log('hello')).close().end();
  }, 30000);
  it('Calling Puppeteer specific functions when chained: dialog', async () => {
    await differencify.init().newPage().on('dialog', async dialog => {
      expect(dialog.message()).toEqual('1');
      await dialog.dismiss();
    }).evaluate(() => alert('1')).close().end();
  }, 30000);
  it('Continue on chained object', async () => {
    await differencify.init().newPage().goto('http://example.com/').mainFrame().then.url().result(url => {
      expect(url).toEqual('http://example.com/');
    }).close().end();
  }, 30000);
  it('Multiple toMatchSnapshot on chained object', async () => {
    await differencify.init().newPage().goto('http://example.com/').waitFor(1000).screenshot().toMatchSnapshot().result(result => {
      expect(result).toEqual(true);
    }).goto('http://example.com/').waitFor(1000).screenshot().toMatchSnapshot().result(result => {
      expect(result).toEqual(true);
    }).close().end();
  }, 30000);
  ...
  it('Custom test path', async () => {
    const customDifferencify = new _index2.default({
  ...
  it('Freeze image in page', async () => {
    await differencify.init().newPage().setViewport({ width: 1600, height: 1200 }).goto('https://i.giphy.com/media/xTiTnoUnHxVaaVNWhO/giphy.webp').waitFor('body > img').freezeImage('body > img').screenshot().toMatchSnapshot().close().end();
  }, 30000);
  it('simple with mock requests', async () => {
    await differencify.init().newPage().mockRequests().setViewport({ width: 1600, height: 1200 }).goto('http://example.com/').waitFor(1000).screenshot().toMatchSnapshot().close().end();
  }, 30000);
  ...
  it('mock requests and replace image', async () => {
    await differencify.init().newPage().mockRequests({ replaceImage: true }).setViewport({ width: 1600, height: 1200 }).goto('https://nimasoroush.github.io/differencify/').waitFor(1000).screenshot().toMatchSnapshot().close().end();
  }, 30000);
  it('mock requests and replace image', async () => {
    process.env.CI = 'true'; // This will simulate CI/CD environment
    await differencify.init().newPage().mockRequests({ replaceImage: true }).setViewport({ width: 1600, height: 1200 }).goto('https://nimasoroush.github.io/differencify/').waitFor(1000).screenshot().toMatchSnapshot().close().end();
